...
addEvent("ptp:onCountdown", true)
addEventHandler("ptp:onCountdown", resourceRoot, function(seconds)
    if countdownActive == false then
        -- enableCountdown(seconds)
        -- addEventHandler("onClientRender", root, onRender)
    end
    outputDebugString("Countdown: " .. tostring(seconds) .. " seconds remaining")
    remaining = seconds
    local text
    if remaining > 0 then
        if remaining <= 3 then
